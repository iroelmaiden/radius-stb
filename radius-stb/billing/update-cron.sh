...
// 1. Bersihkan stale session (session aktif > 60s tanpa update)
$stale = $conn->query("
...
");
$stale_count = $stale->affected_rows;
...
$validity_expire = $conn->query("
    ...
    AND valid_until < NOW()
");
$validity_count = $validity_expire->affected_rows;
...
");
$delete_count = $old->affected_rows;
