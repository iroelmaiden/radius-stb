...
if [[ -f "$HEADER_PHP" ]]; then
    ...
    sed -i 's|href="[^"]*bootstrap[^"]*css[^"]*bootstrap\.min\.css"|href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"|g' "$HEADER_PHP" 2>/dev/null || true
    sed -i 's|href="[^"]*font-awesome[^"]*fontawesome\.min\.css"|href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css"|g' "$HEADER_PHP" 2>/dev/null || true
    log "Header CSS fixed (CDN)"
fi

if [[ -f "$FOOTER_PHP" ]]; then
    sed -i 's|src="[^"]*bootstrap[^"]*js[^"]*bootstrap\.bundle\.min\.js"|src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"|g' "$FOOTER_PHP" 2>/dev/null || true
